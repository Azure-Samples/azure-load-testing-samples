...
Write-Output "`nTotal VUH(Virtual User Hours) needed to perform this test will be - $VUH"
    ...
    try {
        ...
        if($secrets.Length -gt 10)
        {
            ...
            az keyvault set-policy --name $keyvaultname --resource-group $keyvaultresourcegroup --object-id (az load show --name altresource_$altresourcename --resource-group $resourcegroup --query identity.principalId -o tsv) --secret-permissions get list
        }
        else {
            $secretnew = ""
        }    
